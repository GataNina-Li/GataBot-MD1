...
export async function tiktokdl(url) {
    ...
    const decodeParams = (_b = (_a = html.split('))</script>')[0]
    ...
    if (!Array.isArray(decodeParams) || decodeParams.length !== 6)
        throw new ScraperError(`failed to parse decode params!\n${html}`);
    const decode = decodeSnapApp(...decodeParams);
    const result = (_e = (_d = (_c = decode.split('; elem.innerHTML = \\\'')) === null || _c === void 0 ? void 0 : _c[1].split('\\\'; parent.ga(')) === null || _d === void 0 ? void 0 : _d[0]) === null || _e === void 0 ? void 0 : _e.replace(/\\(\\)?/g, '');
    if (!result)
        throw new ScraperError(`failed to parse html from decode!\n${decode}`);
...
export async function tiktokdlv2(url) {
...
export async function tiktokdlv3(url) {
    const body = {
        url: encodeURI(url)
    };
    ...
    if (!no_watermark)
        throw new ScraperError('Failed to download tiktok video!');
    return {
        video: {
            no_watermark
        }
...
export async function tiktokfyp() {
