...
export async function instagramStory(name) {
    ...
    const cookie = resKey.headers['set-cookie'].map(v => v.split('; ')[0].trim()).join('; ');
    const headers = {
...
export async function instagramStoryv2(name) {
    ...
    const { user, medias: results, error } = await got('https://www.instagramsave.com/system/action.php', {
        form: payload,
        method: 'POST',
        headers: {
            ...headers,
    ...
    if (error || !results)
        throw new ScraperError(`Maybe user ${name} not have story!!\n${JSON.stringify({ user, results, payload }, null, 2)}`);
    return {
        user,
