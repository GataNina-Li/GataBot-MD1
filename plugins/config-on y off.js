...
conn.sendButton(m.chat, `${rg}*_COMANDO_* *|* ${type} 
*_ACTUALMENTE*_ *|* ${isEnable ? '✅ ACTIVADO' : '❌ DESACTIVADO'} 
*_EN ESTE*_ *|* ${isAll ? 'BOT' : isUser ? '' : 'CHAT'}`, wm, null, [[`${isEnable ? '🔕 DESACTIVAR' : '🔔 ACTIVAR'}`, `${isEnable ? `.off ${type}` : `.on ${type}`}`], ['👾 𝙼𝙴𝙽𝚄 𝙿𝚁𝙸𝙽𝙲𝙸𝙿𝙰𝙻 👾', '.menu']],m)}
