export function before(m) {
    ...
    if (user.afk > -1) {
        m.reply(`
  ${lenguajeGB['smsAvisoEG']()}𝘿𝙀𝙅𝘼𝙎𝙏𝙀 𝘿𝙀 𝙀𝙎𝙏𝘼𝙍 𝙄𝙉𝘼𝘾𝙏𝙄𝙑𝙊 *(𝘼𝙁𝙆)*${user.afkReason ? ' 𝙀𝙇 𝙈𝙊𝙏𝙄𝙑𝙊 𝘿𝙀 𝙄𝙉𝘼𝘾𝙏𝙄𝙑𝙄𝘿𝘼𝘿 𝙀𝙍𝘼: ' + user.afkReason : ''}
...
𝙏𝙄𝙀𝙈𝙋𝙊 𝙄𝙉𝘼𝘾𝙏𝙄𝙑𝙊: *${(new Date - user.afk).toTimeString()}*`.trim())
    ...
    for (let jid of jids) {
        ...
        m.reply(`${lenguajeGB['smsAvisoAG']()}𝙉𝙊 𝙀𝙏𝙄𝙌𝙐𝙀𝙏𝙀𝙎 𝘼 𝙀𝙎𝙏𝙀(𝘼) 𝙐𝙎𝙐𝘼𝙍𝙄𝙊(𝘼)!! 𝙀𝙎𝙏𝘼 𝙄𝙉𝘼𝘾𝙏𝙄𝙑𝙊(𝘼)     
