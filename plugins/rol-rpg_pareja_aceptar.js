...
conn.reply(m.chat, `*NO PUEDES ACEPTAR SI NADIE SE HA DECLARADO, DECLARATE CON ${ number ? tu : `@${user.split('@')[0]}`} PARA DIGA SI TE ACEPTA O TE RECHAZA*`, m, { contextInfo: { mentionedJid: [user]}})
}else{
global.db.data.users[m.sender].pasangan = user
conn.reply(m.chat, `*Felicitaciones, oficialmente están saliendo. ${ number ? tu : `@${user.split('@')[0]}`}*\n\n*Que dure para siempre y siempre sea feliz ${ number ? tu : `@${user.split('@')[0]}`} 💓 ${ number ? yo : `@${me.split('@')[0]}`} 🥳🥳🥳*`,m, { contextInfo: { mentionedJid: [user, tu, yo]}})
}}	
}
