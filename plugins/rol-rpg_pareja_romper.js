let handler = async (m, { conn }) => {
  ...
  var user = conn.getName(m.sender)

  if(ayg.pasangan == ""){
    return conn.reply(m.chat,`No tienes pareja.`,m)
  }
  if (typeof beb == "undefined"){
    conn.reply(m.chat,`Rompió con éxito con ${user}`,m,{contextInfo: {
      mentionedJid: [user]
    }})
    ayg.pasangan = ""
  }
  ...
  if (m.sender == beb.pasangan){
    conn.reply(m.chat,`Usted Rompió con éxito con ${user}`,m,{contextInfo: {
      mentionedJid: [user]
    }})
    ayg.pasangan = ""
    beb.pasangan = ""
